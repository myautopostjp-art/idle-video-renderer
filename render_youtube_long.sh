...
#
# 【21から17に下げた理由】
# 完成品の同じ絵を比べたところ、ループ本編だけ細かさが4割落ちていた。
#   置き換え区間(CRF14): 2.87
#   ループ本編(CRF21)  : 1.81
# 同じ映像の同じ位相なので、差は圧縮の強さだけによるもの。
# 導入部から本編に移る瞬間に画質が落ちて見える原因がこれ。
#
# 17まで下げると、その差がほぼ埋まる。
#
# 【容量について】
# ループ本編は1周期(約18秒)だけを圧縮し、あとはコピーで並べている。
# CRF17でも1時間で1.5GB前後に収まる見込み。
# Driveが保存先なので問題ない(Releaseの2GB上限には近づく)。
#
#   14 … 中間ファイルと同じ。ほぼ劣化なしだが容量が大きい
#   17 … 差がほぼ分からない(現在)
#   21 … 明らかに眠くなる
FINAL_CRF=17
FINAL_PRESET=slow
...
#
# 【4から12に引き上げた理由】
# 4%は6秒クリップ時代の値。6秒で約3%だったので、
# 測定が大きく外れたときの歯止めとして4%を置いていた。
#
# クリップを20秒にするとドリフトは3倍以上たまるため、
# 8〜10%に達することがある。4%のままだと切り詰められてしまい、
# 半分も打ち消せない。
#
# 【引き換えになるもの】
# 打ち消すぶんだけ画角を切り取るので、ズームが大きいほど
# 実解像度が落ちる。10%のドリフトを消すには10%強を切り取ることになる。
# ログの「拡大○%」がこの値。12%を超えるようなら、
# 補正で押し切るより、クリップを作り直したほうがよい。
DRIFT_MAX_ZOOM_PCT=12
...
#   0 … 従来どおり(始点と終点だけ)
#   1 … 途中の揺れも打ち消す(現在)
# 【0にした理由 — 追加したが、かくかくの原因になった】
# 周期の中の揺れを12箇所測り、その軌跡どおりに画面をずらす処理。
# 狙いは正しかったが、切り取り位置が整数単位でしか動かないため、
# 測った点ごとに階段状に飛ぶ映像になった。
#
# 完成品を1コマずつ測ると、約2.5秒間隔で大きな飛びが出ていた:
#   30.60秒 2.48 / 33.28秒 2.58 / 35.68秒 3.13 / 38.24秒 3.21
#   (平均は0.41なので、6〜8倍の飛び)
#
# 滑らかに動かすには小数単位で動かす必要があり、
# そのためには全体を数倍に拡大してから処理することになる。
# 処理時間が大幅に増えるため、いまは使わない。
#
#   0 … 使わない(現在)
#   1 … 使う。かくかくする
DRIFT_PATH_ENABLED=0
...
if [ "${DRIFT_CORRECTION_ENABLED:-1}" = "1" ]; then
  ...
  for ((i=0; i<CLIP_COUNT; i++)); do
    ...
    if [ -n "$BEST_LABEL" ]; then
      ...
      if [ "${DRIFT_PATH_ENABLED:-0}" = "1" ]; then
...
# 【51から46に戻した】
# 51は雲海を全部止めるために上げた値。
# 高評価だった状態は46だったので、そこに戻す。
SKY_FREEZE_HEIGHT=46

# 雲海の混ぜ具合
#
# 【0.6にした理由 — 導入部とループを同じ「かなりゆっくり」に揃える】
# 導入部の終盤を減速させた結果、導入部の終わりの雲(1.24)と
# ループ素材の生の雲(1.31)がほぼ同じ速さになった。
# ここから両方を同じ割合で落とせば、同じ速さになる。
#
# 目標を0.5(かなりゆっくり)とすると、両方とも約0.4倍にすればよい。
# ループ側は6割止めて4割動かす。導入部側は減速の最終段で合わせる。
#
#   1.0 … 完全に止まる
#   0.6 … 4割動く。導入部の減速と釣り合う(現在)
#   0.0 … 元の速さのまま
# 【0にした理由 — 導入部の雲が流れるようになったため】
#
# 導入部のプロンプトに雲海の動きを書き加えたところ、
# 導入部の雲が自分で流れるようになった。実測:
#
#          雲海   地上
#   13.0秒 5.77   2.49   ← 雲が地上を上回る = 雲自身が流れている
#   16.4秒 3.12   1.84   ← カメラが止まっても動きが残る
#
# 以前は雲海と地上が同じ値に収束していた(雲は静止し、
# カメラが動くから流れて見えていただけ)。
#
# 導入部の雲が3.1で流れているのに、ループ側を0.3で混ぜると
# 0.95まで落ちて速さが合わない。混ぜるのをやめて素の速さで流す。
#
# 【計算】
# 導入部の雲海3.12には、カメラの動きぶん(地上1.84)が含まれる。
# 差し引くと雲自身の動きは約1.3。
# ループ側は混ぜるのをやめると約1.4。ほぼ一致する。
#
#   0   … 混ぜない。素材の速さのまま(現在)
#   0.3 … 3割止める。導入部より遅くなりすぎた
#   1.0 … 完全に止める
SKY_SLOW_RATIO=0

# 星を完全に止める高さ(画面上端からの割合 %)
#
# 【41から22に下げた理由 — 雲海まで止めてしまっていた】
# 完成品を横帯ごとに測ったところ、こうなっていた:
#
#   高さ      導入部の終わり   ループ
#   0〜10%    1.95            0.06  ← 完全静止
#   10〜20%   2.14            0.06  ← 完全静止
#   20〜30%   2.28            0.06  ← 完全静止
#   30〜40%   2.93            0.06  ← 完全静止
#   40〜50%   3.31            0.95
#
# 41%まで完全に止めていたため、そこにある雲海まで巻き込んでいた。
# 導入部では同じ帯が2〜3で動いているので、切り替わった瞬間に
# 空が丸ごと死んで見える。
#
# 実際の画で明るさを測ると、35%から急に明るくなる。そこが雲海の上端。
# 星空は0〜33%程度なので、30%までに絞る。
# それより下(雲海)は SKY_SLOW_RATIO でゆっくり流す。
#
#   30 … 星だけを止める(現在)
#   41 … 雲海まで止めてしまう(以前。空が死んで見えた)
SKY_STAR_FREEZE=30
...
#
# 【0にした理由】
# 実際の星空は、肉眼でも映像でもほとんど瞬かない。
# 描いた光点が明滅すると、かえって作り物らしく見える。
# 元の星の上に重なると、その部分だけ不自然になる場所もあった。
#
# もともとは「空を完全に止めると死んだ画に見える」対策だった。
# しかし雲海と湯が動くようになった今は、その心配がない。
#
#   0 … 描かない(現在)
#   1 … 描く
SKY_TWINKLE_ENABLED=0

# 描く星の数
#   40 … 多すぎて騒がしく見えた
#   20 … 瞬きは分かるが主張しない(現在)
SKY_TWINKLE_COUNT=20
...
#
# そこで導入部の最後の数秒を捨て、代わりにループの映像を置く。
# 切り替わる直前にはすでにループの湯気が流れている状態になり、
# 勢いの段差が消える。
#
# 【どこから切り出すか — 一度失敗している】
# 最初はループの「末尾」から取ったが、そこは継ぎ目を溶かすために
# 先頭と重ねてある区間だった。2枚が重なって動きが二重に見え、
# 完成品を測ると、その4秒だけ動きが2〜3倍に跳ね上がっていた。
#
#   置き換え区間: 動き 4.4 / ループ本編(同じ位相): 動き 1.2
#
# そこで重なりの手前(XFADE_LOOP秒ぶん手前)から切り出す。
# 素の映像なので二重にならず、しかもループの先頭に向かって
# 進んでいく区間なので、動きの流れも自然に繋がる。
#
#   0 … 置き換えない
#   4 … 最後の4秒を置き換える(現在)
# 【0にした理由】
# GAS側で「つなぎの動画」を生成する方式に切り替えたため、
# 導入部の時点で湯気が繋がった状態になっている。
# レンダリング側で置き換える必要がなくなった。
#
# つなぎを使わない場合(HANDOVER_CLIP_SEC=0)は、
# ここを4に戻すと従来の置き換えが働く。
INTRO_HANDOVER=0
...
# 【3から1.5に減らした理由】
# 冒頭3秒を切ると、室内の場面がほとんど残らない。
# 実際、完成品では0.5秒で室内、2秒でもう露天風呂になっており、
# そこから3秒切り落とすと室内が消えていた。
#
# 急旋回の対策として入れた設定だが、切りすぎると
# 「内装を楽しんでから風呂へ向かう」という筋立てが失われる。
INTRO_HEAD_CUT=1.5
...
for ((i=0; i<CLIP_COUNT; i++)); do
  ...
  if [ "$i" -eq 0 ] && [ -f "loop_head_$i.mp4" ]; then
    ...
    if awk "BEGIN{exit !($INTRO_HANDOVER > 0.05)}"; then
      ...
      # 【重なり区間を避ける】
      # ループの末尾 XFADE_LOOP 秒は、先頭と溶け合わせてある。
      # そこを使うと動きが二重に見えるため、その手前から切り出す。
      HAND_MARGIN=$(awk -v x="$XFADE_LOOP" 'BEGIN{printf "%.3f", x}')
      if [ -n "$UNIT_DUR" ] && awk -v u="$UNIT_DUR" -v h="$INTRO_HANDOVER" -v m="$HAND_MARGIN" 'BEGIN{exit !(u > h + m + 0.5)}'; then
        HAND_FROM=$(awk -v u="$UNIT_DUR" -v h="$INTRO_HANDOVER" -v m="$HAND_MARGIN" 'BEGIN{printf "%.3f", u - h - m}')
        # 【完成品と同じ設定で作る】
        # ここだけ中間ファイル用の高画質設定で作ると、
        # ループ本編に移った瞬間に画質が落ちて見える。
        if ffmpeg -y -ss "$HAND_FROM" -i "stage_unit_$i.mp4" -t "$INTRO_HANDOVER" -an \
          ...
          HANDOVER_OK=true
          echo "  段階1: 導入部の最後${INTRO_HANDOVER}秒を、ループの映像(${HAND_FROM}秒地点から)に置き換えます"
          echo "         継ぎ目の重なり(末尾${HAND_MARGIN}秒)は避けています"
        else
...
# 【0にした理由 — 導入部の雲が流れるようになり、不要になった】
# 導入部の雲が静止していたとき、継ぎ目だけでも揃えるための細工だった。
# いまは導入部の雲が自分で流れているので、全体を通して速さが揃う。
# 継ぎ目だけ差し替えると、かえってそこで不連続が生じる。
INTRO_MATCH_SEC=0
...
# 音が「開ける」瞬間
#
# 導入部のどこで湯船が現れるかに合わせて、効果音を立ち上げる。
#
# 【AIが決めた値があればそれを使う】
# カメラワークは生成のたびに変わる。室内から歩き出す構図もあれば、
# 最初から湯船のそばにいる構図もある。決め打ちの秒数では合わない。
# そこで導入部の動画をGeminiに見せ、水が画面に現れる秒数を答えさせ、
# その値をここに渡している(AUDIO_OPEN_START / AUDIO_OPEN_DURATION)。
#
# 値が渡ってこない場合は、従来どおり「導入部の終わり8秒前から」にする。
OPEN_DURATION="${AUDIO_OPEN_DURATION:-8}"
if [ -n "${AUDIO_OPEN_START:-}" ]; then
  OPEN_START="$AUDIO_OPEN_START"
  echo "AIが判断した水の登場位置に音を合わせます"
else
...
if [ "$HAS_AMBIENT" = true ]; then
  ...
  case "$PARTICLE_KEY" in
    onsen)
      ...
      #
      # 【0.40から0.55に上げた】
      # 温泉の情景では湯の音が主役に近い。控えめすぎると
      # 画面で水が流れているのに存在感がなく、物足りなく感じられる。
      #
      #   0.40 … BGMの後ろに完全に回る。控えめすぎた
      #   0.55 … BGMと並び立つ。湯の音がはっきり聴こえる(現在)
      #   0.70 … 湯の音が主役になる。BGMが伴奏に回る
      AMBIENT_LOOP_VOLUME=0.55
      ;;
    *)
  ...
  # 始まりのBGMで残す帯域の上限。下げるほど編成が薄く聴こえる
  # 室内から歩き出す構図では、こもった音から開ける演出が効く。
  BGM_INDOOR_LOWPASS=3000
  BGM_INDOOR_VOLUME=0.55     # 始まりのBGM音量(屋外は0.8)
...
[ind][outd]amix=inputs=2:duration=longest:normalize=0[out]" \
  ...
  #
  # 湯船が画面に無いあいだの音量。
  # 建物の奥から歩いてくる構図なら0(完全な無音)が自然だが、
  # 最初から湯船が見えている構図では0だと不自然になる。
  # AIが「最初から水が見えている」と判断した場合は、この値が上がる。
  DIST_FAR_VOL="${AUDIO_FAR_VOLUME:-0}"
