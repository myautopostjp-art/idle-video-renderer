...
#
# 【0.8に上げた理由】
# 導入部は等速なので、ループが遅いほど切り替わりでテンポの差が目立つ。
# 空を止めた今は雲を気にする必要がないため、速度を上げてテンポを寄せる。
#
#   1.0 … テンポ差ゼロ。ただし6秒のクリップでは周期が短すぎる
#   0.8 … テンポ差が半分に縮む。周期は7.6秒(現在)
#   0.6 … 周期は10.2秒と長いが、テンポ差が目立つ
#
# 【本来の解決】
# 6秒のクリップを引き伸ばして周期を稼いでいるのが、そもそもの無理。
# GASの YOUTUBE_STAGE_CLIP_DURATION_SEC を20にして20秒のクリップを作れば、
# 等速のまま周期20秒が得られ、テンポ差も中間フレームの生成も不要になる。
# そのときは、この値を 1.0 にすること。
#
# ※この値を変えたら render.yml の LOOP_PERIOD も直すこと
#   (0.8なら周期は約5.6秒 = 7.6秒 − 重ねる2秒)
LOOP_SPEED=0.8
...
#
# 【32から45に広げた理由】
# 星空は止まったが、雲海が固定範囲より下にあったため
# まだ流れて見えていた。速度を下げると湯まで遅くなってしまうので、
# 固定する範囲を下げて雲海まで含める。
#
# 建物や稜線がこの範囲に入っても問題ない。動かないものは
# 映像側と完全に同じ位置にあるため、差が生じないため。
# 気をつけるのは、湯や湯気がこの高さまで届いていないかだけ。
#
#   32 … 星空のみ。雲海が流れて見えた
#   45 … 雲海まで含める(現在)
#   55 … さらに下まで。建物の中ほどまで止まるので、
#         湯気が届くようなら行き過ぎ
SKY_FREEZE_HEIGHT=45

# その下の、徐々に映像へ戻していく帯の幅(%)
# 広いほど境目が分からなくなるが、その帯では
# 止まった絵と動く絵が混ざるため、湯気があると二重に見える
SKY_FREEZE_FEATHER=12
