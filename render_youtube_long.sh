...
#
# 【0にした理由】
# 残像は「2枚を重ねている間」にしか出ない。溶かすのをやめれば原理的に消える。
# 導入部の終盤はカメラがまだ止まりきらず前進を続けており、
# LTXは終端の数秒で画が崩れやすい。そこを動かない画と重ねると、
# どれだけ短くしても残像として見えてしまう。
#
# 枠を揃えてあるので、ループの1コマ目は導入部の最終フレームとほぼ同じ絵。
# 直結しても位置は飛ばない。切り替わりで変わるのは
# 「空が止まること」と「テンポ」だけになり、一瞬の変化として処理される。
#
#   0 … 溶かさずに直結する(現在)
#   2 … 二重になる時間を最小限にする。それでも残像が出た
#   4 … ぶれとして見えた
XFADE_INTRO=0
