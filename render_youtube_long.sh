...
#   0.0 … 元の速さのまま
# 【0.3にした理由 — 切り替わり直前の雲の速さを、ループで引き継ぐ】
#
# これまで「導入部の途中(8秒)」と「ループ」を比べて10倍の差と言っていたが、
# 比べる場所が間違っていた。導入部の途中はカメラが動くので速くて当然。
# 見るべきは切り替わりの瞬間。
#
# 実測(切り替わり付近を1秒ごと):
#   16.0秒: 雲2.70  16.5秒: 雲1.12  17.0秒: 雲0.62(ループ開始)
# 雲は滑らかに減速して1.12まで来ている。ところがループに入ると
# 0.60で止まったままになる。導入部で動いていた雲が急に死んで見える。
#
# ループの雲を「切り替わり直前の速さ(1.12)」に近づければ繋がる:
#   混ぜる0.3 → 雲0.99 湯気1.35   ← ほぼ繋がり、雲は湯気より遅い
#   混ぜる0.5 → 雲0.82
#   止める1.0 → 雲0.60           ← 急に止まる(以前)
#
# 導入部には一切触らない。変えるのはここだけ。
#
#   0.3 … 切り替わりの速さを引き継ぐ(現在)
#   0.5 … もう少しゆっくり
#   1.0 … 完全に止める(雲が死んで見える)
SKY_SLOW_RATIO=0.3
