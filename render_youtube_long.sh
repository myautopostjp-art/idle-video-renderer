...
#
# 【実際の画面を測って決めた値】
# onsen_night_terrace の構図を測ったところ、こうなっていた:
#   0〜41%  星空
#   41〜47% 山の稜線
#   47〜58% 雲海          ← 止めたいのはここまで
#   60%〜   石のテラス・灯籠
#   66%〜   湯船          ← ここから下は動いていてほしい
#
# 45%にしていたときは、雲海がまるごとぼかしの帯(45〜57%)に入っていた。
# 半分だけ動く中途半端な状態になり、止まりきらないのに自然でもない、
# いちばん落ち着かない見え方になっていた。
#
# 58%まで完全に固定すれば雲海は止まり、湯船(66%〜)には掛からない。
# 途中のテラスや灯籠はもともと動かないので、固定しても差は出ない。
#
# ※テーマを変えたら構図も変わる。この値も測り直すこと
SKY_FREEZE_HEIGHT=58

# その下の、徐々に映像へ戻していく帯の幅(%)
# 58〜63%はテラスの石。動かないものなので、狭くても境目は見えない
SKY_FREEZE_FEATHER=5
